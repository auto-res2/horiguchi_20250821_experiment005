...
\begin{itemize}
    ...
    \item \textbf{Mixing-Time Drift} Heuristic $k$-schedules embed unverified assumptions on mixing time that break down as learning proceeds [decelle_2021_equilibrium].
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Strict Complex-Unitary Training} Householder stacks with Riemannian-Adam double capacity over real-orthogonal RBMs and bound singular values exactly [bachtis_2024_cascade].
...
\textit{Unbiased Deep Boltzmann Machines}  Recent unbiased contrastive-divergence schemes achieve FID\,≈\,10 on MNIST but rely on Metropolis–Hastings couplings and do not provide associative memory [taniguchi_2023_end]. MU-SH-RBM instead targets shallow models with one-pass recall and minimal compute, serving as a drop-in replacement for teaching codebases.
...
\textbf{Environment}  Python\,3.11, PyTorch\,2.1, CUDA\,11.8, cuDNN\,8.9. All third-party packages are installed as recorded in the experiment log. A global RNG seed (17) is written to each checkpoint; Git commit hashes for all repositories are logged.
...
\textbf{Generative Quality}  With only 500 generated samples MU-SH-RBM attains FID\,=\,15.8 and diversity\,=\,0.4331 while averaging $\bar k = 9$ Gibbs steps. Baseline figures are FID\,≈\,18.9 at $k=25$ for Conv-RBM and FID\,≈\,21.6 at $k=1$ for HA-RBM; thus MU-SH-RBM sits on the favourable upper-left quadrant of the quality–cost plane.

\textbf{One-Shot Denoising}  Because the demonstration omits the full wavelet-domain reconstruction, PSNR stagnates at \(\approx 20\,\text{dB}\) regardless of dropout—well below the 33\,dB achieved by the full model. Nevertheless, associative recall accuracy is 100\,\% up to 50\,\% corruption, proving that the energy landscape stores patterns faithfully.
